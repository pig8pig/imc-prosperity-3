...
class DynamicTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # Calculate Volume Weighted Mid Price (VWAP) of the book and use it as a proxy for fair value instead of wall_mid
        best_bid = self.best_bid
        best_ask = self.best_ask
        best_bid_volume = self.mkt_buy_orders[best_bid]
        best_ask_volume = self.mkt_sell_orders[best_ask]
        # fair value is cross weighted prices as if there are more asks then bids, then the fair value is closer to the best bid
        fair_value = (best_bid * best_ask_volume + best_ask * best_bid_volume) / (best_bid_volume + best_ask_volume)

        # Calculate skew as a measure of order book imbalance
        # spread is a free parameter that controls how much the skew influences our orders — we can tune this based on how aggressively we want to follow Olivia
        spread = 1
        skew = (self.initial_position/ self.position_limit) * (spread)

        bid_price = round(fair_value - spread - skew)
        ask_price = round(fair_value + spread - skew)

        # If the market is already offering better than our theoretical bid, just take it
        if self.best_ask <= bid_price:
            self.bid(self.best_ask, self.max_allowed_buy_volume)
        else:
            self.bid(bid_price, self.max_allowed_buy_volume)

        # If the market is already bidding higher than our theoretical ask, just hit it
        if self.best_bid >= ask_price:
