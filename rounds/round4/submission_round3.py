...
# ─────────────────────────────────────────────────────────────────────────────
# StaticTrader per-product config — fair value uses microprice; overbid/undercut
# is preserved and inventory-skewed; Bollinger entries fire BEFORE taking logic.
# HYDROGEL constants validated from trader4_nick.py (BOLL_WINDOW reduced from
# 10000 → 1000). VELVET constants are starting guesses pending backtest sweep.
# ─────────────────────────────────────────────────────────────────────────────
STATIC_CONFIG = {
    'HYDROGEL_PACK': {
        'BOLL_WINDOW': 1000,
        'BOLL_Z_THRESHOLD': 1.5,
        'BOLL_TRADE_CAP': 30,
        'SKEW_FACTOR': 0.05,
        'SPREAD': 5,
    },
...
# ─────────────────────────────────────────────────────────────────────────────
# CounterpartyTrader config — Round 4 unlocks named counterparties on
# Trade.buyer / Trade.seller. Mark 14 is a consistent winner (+42k across 3
# days on HYDROGEL_PACK + VEV_4000); Mark 38 is the mirror loser (-34k). All
# other Marks (01, 22, 49, 55, 67) are mixed and intentionally NOT used.
# ─────────────────────────────────────────────────────────────────────────────
WINNERS = {
...
COUNTERPARTY_TRADE_SIZE = 30
COUNTERPARTY_MIN_FLOW = 5
...
class StaticTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # Volume-weighted toward the thinner side (best_bid * ask_vol +
        # best_ask * bid_vol) / (bid_vol + ask_vol). Falls back to wall_mid
        # if either top-of-book side is missing or volumes sum to zero.
        if (self.best_bid is not None
                and self.best_ask is not None
                and self.best_bid in self.mkt_buy_orders
                and self.best_ask in self.mkt_sell_orders):
            bid_vol = self.mkt_buy_orders[self.best_bid]
            ask_vol = self.mkt_sell_orders[self.best_ask]
            denom = bid_vol + ask_vol
            fair = ((self.best_bid * ask_vol + self.best_ask * bid_vol) / denom
                    if denom > 0 else self.wall_mid)
        else:
            fair = self.wall_mid

        # ─── Bollinger price-window persistence ─────────────────────────────
        boll_key = f"BOLL_{self.name}"
        boll_state = self.last_traderData.get(boll_key, {}) or {}
        prices = list(boll_state.get('prices', []))
        prices.append(fair)
        if len(prices) > cfg['BOLL_WINDOW']:
            prices = prices[-cfg['BOLL_WINDOW']:]
        # Persist before any early returns so the window keeps growing.
        self.new_trader_data[boll_key] = {'prices': prices}

        ##########################################################
        ####### 0. BOLLINGER — aggressive entries (FIRST DIBS)
        ##########################################################
        if len(prices) >= BOLL_WARMUP:
            arr = np.asarray(prices, dtype=float)
            mean = float(arr.mean())
            std = float(arr.std())
            z = (fair - mean) / max(std, 0.001)

            if z < -cfg['BOLL_Z_THRESHOLD'] and self.best_ask is not None:
                avail = self.mkt_sell_orders.get(self.best_ask, 0)
                size = min(cfg['BOLL_TRADE_CAP'], self.max_allowed_buy_volume, avail)
                if size > 0:
                    self.bid(self.best_ask, size, logging=False)
            elif z > cfg['BOLL_Z_THRESHOLD'] and self.best_bid is not None:
                avail = self.mkt_buy_orders.get(self.best_bid, 0)
                size = min(cfg['BOLL_TRADE_CAP'], self.max_allowed_sell_volume, avail)
                if size > 0:
                    self.ask(self.best_bid, size, logging=False)

        ##########################################################
        ####### 1. TAKING — aggressive orders that cross fair
        ##########################################################
...
# ─────────────────────────────────────────────────────────────────────────────
# CounterpartyTrader — follow Mark 14, fade Mark 38
# Round 4 unlocks Trade.buyer / Trade.seller. For one symbol per instance:
#   • Sum signed informed_flow from market trades touching named winners/losers
#   • Cross the spread when |informed_flow| ≥ COUNTERPARTY_MIN_FLOW
# Honors `reserved_orders` (orders already produced by upstream traders this
# tick) by debiting their committed quantities from buy/sell capacity, so we
# never blow through position limits when other traders also hit the symbol.
# ─────────────────────────────────────────────────────────────────────────────
class CounterpartyTrader:

    def __init__(self, state, prints, new_trader_data, symbol, reserved_orders=None):
        self.state = state
        self.prints = prints
        self.new_trader_data = new_trader_data
        self.symbol = symbol
        self.reserved_orders = reserved_orders or {}

    def _log(self, kind, payload, group="COUNTERPARTY"):
        bucket = self.prints.get(group, {})
        bucket[f"{self.symbol}:{kind}"] = payload
        self.prints[group] = bucket
    ...
    def _run(self):
        pt = ProductTrader(
            self.symbol, self.state, self.prints, self.new_trader_data,
            product_group='COUNTERPARTY',
        )

        # Debit capacity for orders other traders have already committed this tick
        for ord in self.reserved_orders.get(self.symbol, []):
            if ord.quantity > 0:
                pt.max_allowed_buy_volume -= ord.quantity
            else:
                pt.max_allowed_sell_volume -= abs(ord.quantity)
        pt.max_allowed_buy_volume = max(0, pt.max_allowed_buy_volume)
        pt.max_allowed_sell_volume = max(0, pt.max_allowed_sell_volume)

        winners = WINNERS.get(self.symbol, [])
        losers = LOSERS.get(self.symbol, [])

        informed_flow = 0
        trades = self.state.market_trades.get(self.symbol, []) or []
        for trade in trades:
            qty = trade.quantity
            if trade.buyer in winners:
                informed_flow += qty
            if trade.seller in winners:
                informed_flow -= qty
            if trade.buyer in losers:
                informed_flow -= qty
            if trade.seller in losers:
                informed_flow += qty

        action = "NONE"
        size = 0
        if informed_flow >= COUNTERPARTY_MIN_FLOW and pt.best_ask is not None:
            avail = pt.mkt_sell_orders.get(pt.best_ask, 0)
            size = min(COUNTERPARTY_TRADE_SIZE, pt.max_allowed_buy_volume, avail)
            if size > 0:
                pt.bid(pt.best_ask, size, logging=False)
                action = "BUY"
        elif informed_flow <= -COUNTERPARTY_MIN_FLOW and pt.best_bid is not None:
            avail = pt.mkt_buy_orders.get(pt.best_bid, 0)
            size = min(COUNTERPARTY_TRADE_SIZE, pt.max_allowed_sell_volume, avail)
            if size > 0:
                pt.ask(pt.best_bid, size, logging=False)
                action = "SELL"
        ...
        self._log("TICK", {
            ...
            "action": action,
            "size": int(size),
            "pos": pt.initial_position,
        })

        return {self.symbol: pt.orders}
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # 3. CounterpartyTrader for HYDROGEL_PACK — DISABLED (uncomment to enable)
        # try:
        #     cp = CounterpartyTrader(
        #         state, prints, new_trader_data,
        #         symbol='HYDROGEL_PACK',
        #         reserved_orders=result,
        #     )
        #     for sym, ords in cp.get_orders().items():
        #         if ords:
        #             result.setdefault(sym, []).extend(ords)
        # except Exception as e:
        #     logger.print(f"ERROR in CounterpartyTrader[HYDROGEL_PACK]: {e}")

        # 4. CounterpartyTrader for VEV_4000 — DISABLED for initial test (uncomment to enable)
        # try:
        #     cp = CounterpartyTrader(
        #         state, prints, new_trader_data,
        #         symbol='VEV_4000',
        ...
        #             result.setdefault(sym, []).extend(ords)
        # except Exception as e:
        #     logger.print(f"ERROR in CounterpartyTrader[VEV_4000]: {e}")

        # 5. OptionTrader — DISABLED for final submission (was losing -2k on
