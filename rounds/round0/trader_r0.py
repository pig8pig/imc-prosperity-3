...
# ─────────────────────────────────────────────────────────────────────────────
STATIC_SYMBOL = 'EMERALDS'   # stable fair-value product → pure market-making
DYNAMIC_SYMBOL = 'TOMATOES'              # trending product → follow informed trader
...
class DynamicTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # Bid and Ask based on fair value and skew, using all remaining buy capacity
        self.bid(bid_price, self.max_allowed_buy_volume)
        self.ask(ask_price, self.max_allowed_sell_volume)
        return {self.name: self.orders}
